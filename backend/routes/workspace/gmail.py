...
@router.post("/search")
async def gmail_search(body: ToolInvokeRequest, user: CurrentUser) -> ResponseEnvelope:
    return _invoke("gmail.search", body)


@router.post("/message")
async def gmail_get_message(body: ToolInvokeRequest, user: CurrentUser) -> ResponseEnvelope:
    return _invoke("gmail.get_message", body)


@router.post("/thread")
async def gmail_get_thread(body: ToolInvokeRequest, user: CurrentUser) -> ResponseEnvelope:
    return _invoke("gmail.get_thread", body)


@router.post("/draft")
async def gmail_create_draft(body: ToolInvokeRequest, user: CurrentUser) -> ResponseEnvelope:
    return _invoke("gmail.create_draft", body)


@router.post("/send")
async def gmail_send(body: ToolInvokeRequest, user: CurrentUser) -> ResponseEnvelope:
